      ...
      <div class="course-info">
        ...
        <div class="wrap-right">
          ...
          <p class="data">{{ course.students }}人在学&nbsp;&nbsp;&nbsp;&nbsp;课程总时长：{{
              course.lessons
            }}课时/更新课时：{{ course.pub_lessons }}课时&nbsp;&nbsp;&nbsp;&nbsp;
            难度：{{ course.level_name }}
            <!--            <span v-if="course.level==0">入门</span><span v-else-if="course.level==1">进阶</span>-->
            <!--            <span v-else-if="course.level==2">大师</span>-->
          </p>
          <div class="sale-time">
        ...
        <div class="course-tab-list">
          ...
          <div class="tab-item" v-if="tabIndex==2">
            <div class="tab-item-title">
              ...
              <p class="chapter-length">共{{ article.length }}章 {{ course.lessons }}个课时</p>
            </div>
            <div class="chapter-item" v-for="(cap,index) in article" :key="index">
              <p class="chapter-title"><img src="../static/image/12.png" alt="">
                第{{ index + 1 }}章·{{ cap.name }}</p>
              <ul class="lesson-list">
                <li class="lesson-item" v-for="(capp,key) in cap.lesson_list" :key="key">
                  <span><p class="name"><span class="index">{{ index + 1 }}-{{ key + 1 }}</span> {{ capp.name }}</p>
                  <p class="time">{{ capp.duration }} <img src="../static/image/chapter-player.svg"></p>
                  <button class="try">立即购买</button></span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="course-side">
          <div class="teacher-info">
            ...
            <div class="teacher-content">
              <div class="cont1">
                <img :src="this.$settings.HOST + course.teacher.image">
                <div class="name">
...
export default {
  ...
  created() {
    this.get_all_course(this.$route.query.id)
    this.get_all_chapter(this.$route.query.id)
  },
  data() {
    return {
      course_id: 1,
      ...
      playerOptions: {
        ...
        poster: "../static/image/1.jpg", //视频封面图
  ...
  methods: {
    ...
    get_all_course(course_id) {
      console.log(course_id, 2222)
      this.$axios({
        method: 'get',
        url: this.$settings.HOST + 'course/cours/',
        params: {
          id: course_id
        }
      }).then(response => {
        this.course = response.data.data
        console.log(response.data, 194)
        // 设置课程活动的倒计时
