    ...
    <div class="main">
      ...
      <div class="condition">
        <ul class="cate-list">
          ...
          <li @click="category=cate.id" :class="category===cate.id?'this':''" v-for="(cate, index) in category_list" :key="index">{{cate.name}}</li>
        </ul>
      ...
      <div class="course-list">
        ...
        <div class="course-item" v-for="(course, index) in course_list" :key="index">
          ...
          <div class="course-info">
            <h3 @click="details(course.id)">{{ course.name }}<span><img src="../static/image/avatar1.svg" alt="">{{ course.students }}人已加入学习</span></h3>
            <p class="teather-info">{{course.teacher.title}}
              <span>共{{
                  course.lessons
                }}课时/{{ course.lessons == course.pub_lessons ? '更新完成' : `已更新${course.pub_lessons}` }}</span>
            </p>
            <ul class="lesson-list">
              <li v-for="(lesson, key) in course.lesson_list" :key="key"><span class="lesson-title">
               {{ key + 1 }} | 第{{ key + 1 }}节：{{ lesson.name }}</span> <span class="free" v-if="lesson.free_trail">免费</span>
              </li>
            </ul>
            ...
            <div class="pay-box" v-else>
              <span class="discount-price">价格￥：{{ course.price }}元</span>
              <span class="buy-now">立即购买</span>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods:{
    ...
    get_all_course:function (){
      ...
      this.$axios({
        method:'get',
        url:this.$settings.HOST + 'course/courses/',
        params: filters
      }).then(response=>{
